...
from app.schemas.resumes import CandidateDetailRead, ResumeProcessResult, ResumeTextCreate, ScoreRead
...
def _finish_resume_processing(
    ...
    *,
    ...
    profile_data, parser_name = generate_candidate_profile(parsed_text)
...
async def upload_and_process_resume(
    ...
    try:
        ...
        return _finish_resume_processing(
            db,
            candidate=candidate,
            resume=resume,
...
def process_resume_text(
    ...
    resume = resume_repo.create_resume(
        db,
        candidate_id=candidate_id,
        original_filename=payload.original_filename or "boss-preview-resume.txt",
        file_path=None,
    )
    try:
        return _finish_resume_processing(
            ...
            job=job,
            job_id=job_id,
            source=payload.source or "boss_preview",
            ocr_used=False,
        )
    except Exception as exc:
        db.rollback()
        audit_repo.create_audit_log(
            ...
            status="failed",
            detail=f"{type(exc).__name__}: {exc}",
            payload={"candidate_id": candidate_id, "source": payload.source},
        )
        raise HTTPException(status_code=422, detail=f"简历文本处理失败：{exc}") from exc
