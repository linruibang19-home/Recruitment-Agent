...
- 沟通采集支持 `request_resumes_batch` 受控写入任务：每批最多处理 20 个未读/红点会话，仅发送固定索要简历话术。
- 批量索要简历会跳过已有简历附件、已索要过简历或候选人已表达简历已发送的会话，避免重复触达。
- 批量任务会回传发送、跳过和失败统计，便于在真实页面测试。
- 扩展长任务支持暂停、继续和停止，便于真实页面测试。
- 批量读取结果会按候选人入库，未发现简历时生成索要简历待确认草稿，发现 PDF 附件时自动回传到简历解析流程。
- 附件没有 PDF 直链但可预览时，扩展会提取预览文本并进入简历画像和评分流程。
- 除固定索要简历话术外，约面试、打招呼和其它回复仍进入待确认队列。
