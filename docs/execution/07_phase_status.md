...
状态：完成业务化导航重组和 BOSS 页面选择器稳定性增强。

完成内容：

- 前端左侧目录从平铺功能模块调整为业务分组：
  - 招聘工作台：今日概览、每日推荐、待我确认
  - 候选人流程：新招呼处理、候选人库、流程推进
  - BOSS 自动化：推荐牛人
  - 岗位与规则：岗位管理、自动化参数
  - 数据与审计：审计日志
- 移动端导航保留横向滚动，桌面端固定业务分组，突出从今日处理到候选人推进的招聘主线。
- 扩展端增强 BOSS 沟通页识别：
  - 增加聊天列表、消息气泡、候选人姓名、附件卡片的多选择器兜底。
  - 未读识别支持无数字红点。
  - 点击聊天会话改为模拟完整鼠标事件，降低点到内部元素无效的概率。
  - 批量读取和批量索要简历增加“打开会话后等待详情稳定”逻辑，降低读到上一个候选人聊天的概率。
  - 附件预览优先点击“点击预览/预览附件/查看/打开”等子按钮。
  - 固定话术发送同时支持“求简历”和“常用语”入口，并在发送前规范化输入框内容。
- 新增扩展端单元测试覆盖无数字红点和常用语入口发送。

自检结果：

- `python -m compileall -q backend`: 通过。
- `PYTHONPATH=backend pytest -q`: 27 项通过。
- `npm run build`: 通过。
- Playwright 使用本机 Chrome 验证前端：
  - 左侧目录显示业务分组。
  - 点击“新招呼处理”进入 BOSS 候选人流程页。
  - 桌面和移动截图无框架错误，控制台无错误或警告。

下一阶段：

- 在真实 BOSS 页面继续跑批量索要简历，基于失败日志补充特殊弹窗、遮罩、页面懒加载和附件下载场景。
- 将 BOSS 自动化页进一步拆成“沟通页自动化 / 推荐牛人自动化 / 扩展任务日志”三个子视图，减少单页信息密度。

## Phase 13 Status

状态：完成目录树层级增强和批量索要简历闭环可观测性增强。

完成内容：

- 左侧导航进一步强化层级概念：
  - 每个一级业务组增加一句用途说明，例如“每天先看 / 主业务线 / 平台采集 / 配置标准 / 复盘追踪”。
  - 每个子模块增加流程编号 `01-10`，突出从今日概览到审计日志的使用顺序。
- BOSS 候选人流程页新增 4 步使用指引：
  - 打开 BOSS 沟通页
  - 先扫描沟通列表
  - 批量索要简历
  - 检查结果与待确认
- 扩展端批量索要简历增强：
  - 批量任务会滚动聊天列表补足未读目标，避免只处理首屏可见候选人。
  - 每个候选人生成 `execution_trace`，记录暂停控制、打开聊天、附件预览、索要简历等步骤。
  - 发送后会再次读取聊天详情验证固定话术是否出现在沟通记录中。
  - 批次结果增加 `summary` 和 `target_snapshots`，便于在底层执行记录中定位失败原因。

自检结果：

- `python -m compileall -q backend`: 通过。
- `PYTHONPATH=backend pytest -q`: 28 项通过。
- `npm run build`: 通过。
- Playwright 使用本机 Chrome 验证前端：
  - 左侧目录显示一级业务组、用途说明和编号。
  - 点击“新招呼处理”进入 BOSS 候选人流程页。
  - 页面显示 4 步使用指引，控制台无错误或警告。

使用建议：

- 日常先进入“今日概览”看状态，再到“新招呼处理”执行 BOSS 沟通采集。
- 第一次运行先点“扫描沟通列表”，确认扩展能读到未读候选人。
- 确认无异常后再点“批量索要简历”或启动自动循环。
- 任何失败都先看“底层执行记录”的 `execution_trace`，再看“审计日志”。
